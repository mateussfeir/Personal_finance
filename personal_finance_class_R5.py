...
while running:
            ...
            if len(row) > 0:
                if index >= 1:
                    ...
                    if user_choice == '1':
                        ...
                        # 4) Plot the cash flow's chart per day
                        
                        dates = list(transactions_by_date.keys())
                        cash_flows = [sum(values) for values in transactions_by_date.values()]
                        cumulative_cash_flows = []
                        cumulative_value = 0
                        for cash_flow in cash_flows:
                            cumulative_value += cash_flow
                            cumulative_cash_flows.append(cumulative_value)
                        plt.plot(dates, cumulative_cash_flows)
                        plt.xlabel('Date')
                        plt.ylabel('Cumulative Cash Flow')
                        plt.title(f'{MONTH} Cumulative Cash Flow')
                        plt.show()
                    elif user_choice == 'q':
                        running = False
                else:
    ...
    if user_choice == '2':
        ...
        for date, values in transactions_by_date.items():
            ...
            print("{}'s Cash Flow: {:.2f}$".format(date, daily_cash_flow))
